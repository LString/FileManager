...
class DB {
  ...
  initializeSchema() {
    ...
    if (!hasOriginalNumber) {
      this.connection.prepare("ALTER TABLE documents ADD COLUMN original_number TEXT NOT NULL DEFAULT ''").run();
    }
  ...
  convertToImportant(normalUuid) {
    return this.connection.transaction(() => {
      ...
      const insert = this.connection.prepare(`
        INSERT INTO documents (
          uuid, doc_type, type_serial, sender_unit, sender_number, original_number, sender_date,
          input_user, drafting_unit, title, review_leader, secrecy_level,
          crgency_level, secrecy_period, remarks, is_important
        )
        SELECT
          @newUuid, 2, @serial, sender_unit, sender_number, original_number, sender_date,
          input_user, drafting_unit, title, review_leader, secrecy_level,
